class KalmanFilterOneDimension:
    ...
    def __init__(self, initial_value, state_transition_matrix=1.0,
        ...
        self.Q = process_noise_matrix # variance state transition matrix (process noise matrix)
        self.R = measurement_noise # measurement noise covariance matrix (variance observation matrix)
        self.P = estimated_error ** 2 # squared uncertainty of an estimate (covariance matrix)
    ...
    def __update(self, measurement):
        ...
        self.kalman_gain = self.P * self.H / (self.H * self.P * self.H + self.R)
        # https://www.kalmanfilter.net/stateUpdate.html
        self.estimated_value = self.predicted_value + self.kalman_gain * (measurement - self.H * self.predicted_value)
